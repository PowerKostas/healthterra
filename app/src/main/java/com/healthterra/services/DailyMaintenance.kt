    ...
    mutex.withLock {
        withContext(Dispatchers.IO) {
            try {
                ...
                // Syncs to Firestore, before the Room API reset, because the second happens faster, the data is stored in the worker using the
                // snapshots, needs network
                ...
                val syncRequest = OneTimeWorkRequestBuilder<SyncDailyTrackingsWorker>()
                    ...
                    .setInputData(workDataOf(
                        ...
                        "snapshot_date" to userSettings.lastSavedDate // Last saved date's data is being synced, so that date is needed
                    ))
                    .build()
